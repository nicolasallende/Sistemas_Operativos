...
expand_environ_var(char *arg)
{
	if (strstr(arg, "$") != NULL) {
		if (strstr(arg, "?") != NULL) {
			char *numEstado;
			sprintf(numEstado, "%d", status);
			return numEstado;
		}

		strcpy(arg, &arg[1]);  // Le quito el signo $
		char *nuevoArgumento = getenv(
		        arg);  // esto va sin el +1 y con lo de arriba avilitado
		if (nuevoArgumento != NULL && *nuevoArgumento != *"") {
			arg = (char *) realloc(
			        arg, strlen(nuevoArgumento) * sizeof(char));
			strcpy(arg, nuevoArgumento);
		} else {
			arg = (char *) realloc(arg, sizeof(""));
			strcpy(arg, "");
		}
	}

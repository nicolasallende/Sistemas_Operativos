...
uint32_t total_envs_created = 0;
uint32_t total_scheduler_calls = 0; 
uint32_t calls_to_envs_of_each_priority[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
env_run(struct Env *e)
{
	...
	if(curenv != NULL){
		...
		if(curenv->env_priority>1)
			curenv->env_priority--;
	}
	curenv = e;
	e->env_status = ENV_RUNNING;
	e->env_runs++;	// Increment the env_runs counter
	env_load_pgdir(e);
	unlock_kernel();
	...
	panic("context switch failed."); /* mostly to placate the compiler */
}

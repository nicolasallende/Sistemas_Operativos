...
// Returns the env priority of requested enviroment
static int
...
static int
sys_set_env_priority(envid_t envid, int new_priority)
{
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, 1)))
		return r;
	int current_priority = env->env_priority;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (dstenv->env_ipc_dstva) {
		if ((r = env_page_alloc(dstenv, dstenv->env_ipc_dstva, perm)) < 0)
			return r;
		if ((r = env_page_map(curenv, srcva, dstenv, dstenv->env_ipc_dstva, perm)) < 0)
			return r;
